...
class AzureDataLoader:
    ...
    def _initialize_credentials(self):
        ...
        try:
            ...
            # Prefer Service Principal, but verify it can fetch a token; otherwise fall back to CLI, then Default
            if self.client_id and self.client_secret:
                ...
                # Validate SP can obtain a token for ARM (ADF)
                try:
                    sp_cred.get_token("https://management.azure.com/.default")
                    self.credential = sp_cred
                    logger.info("Using Service Principal authentication")
                    return
                except Exception as sp_err:
                    logger.warning(f"Service Principal token acquisition failed: {sp_err}")

            # Add Azure CLI to PATH for this process
            cli_path = r"C:\Program Files\Microsoft SDKs\Azure\CLI2\wbin"
            if os.path.exists(cli_path) and cli_path not in os.environ.get("PATH", ""):
            ...
            try:
                ...
                self.credential = cli_cred
                logger.info("✓ Using Azure CLI authentication (resilient)")
                return
            ...
            try:
                ...
                logger.error(f"DefaultAzureCredential failed to obtain token: {def_err}")
                raise
    ...
    def load_to_processed_container(self, df: pd.DataFrame, file_prefix: str = "nyc_311") -> Optional[str]:
        ...
        try:
            # Generate filename with timestamp
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"{file_prefix}_processed_{timestamp}.csv"
    ...
    def trigger_adf_pipeline(self, 
        ...
        try:
            ...
            # Prepare pipeline parameters, respect provided fileName if present
            pipeline_params = dict(parameters or {})
            
            # Determine target file name: prefer explicit, else discover latest
            # Use provided 'fileName' parameter immediately (skip discovery), if present
            file_name = None
            if 'fileName' in pipeline_params and pipeline_params['fileName']:
                file_name = str(pipeline_params['fileName'])
            if source_file_path:
